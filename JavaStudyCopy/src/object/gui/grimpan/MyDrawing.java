...
public class MyDrawing extends JFrame {
    ...
    MyDrawing() {
    	...
    	p2.add(can);					// 그림그리기 영역에 도화지는 부착시킨다.
    	...
    	can.addMouseMotionListener(my);		// 캔버스 객체에 마우스 모션 리스너를 부착한다.
	...
	class MyHandler implements MouseMotionListener, ActionListener, MouseListener {
		...
		public void mouseDragged(MouseEvent e) {
			...
			// 마우스를 드래그한 지점의 x, y좌표값을 얻어와서 canvas의 x, y좌표값에 전달한다.
			int xx = e.getX();
			int yy = e.getY();
			setTitle("x: " + xx + ", y: " + yy);
