...
public class Bingo extends JFrame {
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(int i=0 ; i < SIZE*SIZE ;i++ ) {
					if((JButton)btn == btnArr[i])  {
						...
						int cnt = 0;	// 2차원 배열의 순서를 1차원으로 설정하는 변수20
		...
		boolean checkBingo() {
			...
			for(int row = 0; row < btnOX.length; row++) {
				for(int col = 0; col < btnOX[row].length; col++) {
					...
					if(row == col && btnOX[row][col] == true) {
						crossCount1++; 
						if(crossCount1 == SIZE) {
							for(int x=0 ; x< btnArr.length ; x +=6 ) {
								// 0부터 24까지 6씩 증가시키면서 색상을 변경
								btnArr[x].setBackground(Color.GREEN);
							}
						}
					}	// 주대각선 검사 끝	
					...
					// 역대각선 검사(우상-좌하단)
					//	1. btxOX[0][4], [2][3], [3][1], [4][0]이 모두 true이면 역대각선 완성
					//	if((btnOX.length-1) - row) == c && btnOX[row][col] == true)
					//  2. row와 col을 더한 값이 4인 것만 검사하면 된다.
					...
					if(row + col == 4 &&  btnOX[row][col] == true ) {
						...
						if(crossCount2 == SIZE) {
							...
							for(int x=1 ; x<SIZE+1 ; x++ ) {
								btnArr[x*4].setBackground(Color.GREEN);
							}
							*/	
						}
					}	// 역대각선 검사 끝	
				...
				// crossCount1과 crossCount2를 초기화 시키지 않으면 이곳에 기술해도 된다.
				// 초기화 시키지 않으면 crossCount1이 증가되면서 SIZE와 같아지는 경우는 1번만 존재하므로,
			...
			// 주대각선, 역대각선은 오직 1개씩만 존재하므로, 검사가 모두 끝난 후에 bingoCount를 증가시킨다.
			if(crossCount1 == SIZE)	bingoCount++;
			if(crossCount2 == SIZE)	bingoCount++;
